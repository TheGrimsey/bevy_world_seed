use bevy::{math::Vec2, prelude::{ReflectDefault, ReflectResource, Resource}, reflect::Reflect};
...
pub struct TerrainNoiseSplineLayer {
    pub amplitude_spline: splines::Spline<f32, f32>,
...
impl TerrainNoiseSplineLayer {
    ...
    pub fn sample(&self, x: f32, z: f32, noise: &Simplex) -> f32 {
        let noise_value = noise.get([(x * self.frequency) as f64, (z  * self.frequency) as f64]) as f32;

        self.amplitude_spline.sample((noise_value / 2.0) + 0.5).unwrap_or(0.0)
    }
}
...
impl TerrainNoiseSettings {
    ...
    pub fn sample_position(&self, noise_cache: &mut NoiseCache, pos: Vec2) -> f32 {
        ...
        self.layers.iter().fold(spline_height, |acc, layer: &TerrainNoiseDetailLayer| {
            acc + layer.sample(pos.x, pos.y, noise_cache.get(layer.seed))
        })
    }
}
