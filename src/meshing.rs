use bevy::{app::{App, Plugin, PostUpdate}, asset::{Assets, Handle}, math::{IVec2, Vec3}, prelude::{Changed, Commands, Entity, IntoSystemConfigs, Mesh, Query, Res, ResMut}, render::{mesh::{Indices, PrimitiveTopology}, render_asset::RenderAssetUsages}};
use bevy_rapier3d::prelude::Collider;

use crate::{terrain::{TerrainCoordinate, TileToTerrain}, update_terrain_heights, Heights, TerrainSettings};

pub struct TerrainMeshingPlugin;
impl Plugin for TerrainMeshingPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(PostUpdate, update_mesh_from_heights.after(update_terrain_heights));
    }
}
...
fn create_terrain_mesh(size: f32, edge_length: u16, heights: &[f32], neighbours: &[Option<&[f32]>; 4]) -> Mesh {
